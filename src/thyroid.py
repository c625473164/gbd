...
def create_result_table(grouped):
    ...
    df_2019 = grouped[grouped['year'] == 2019]
    ...
    for sex in ['Both', 'Male', 'Female']:
        ...
        total_2019 = df_2019[df_2019['sex_name'] == sex]['val'].sum()
        
        # 计算ASIR
        asir_1990 = calculate_asir(sex_data[sex_data['year'] == 1990], 1990)
        asir_2019 = calculate_asir(sex_data[sex_data['year'] == 2019], 2019)
        
        # 添加总计行
        aapc_total = calculate_aapc(total_1990, total_2019)
        results.append({
            'Characteristics': sex,
            'Sex': sex,
            '1990_cases': format_number(total_1990),
            '1990_asir': f"{asir_1990:.2f}",  # 添加ASIR
            '2019_cases': format_number(total_2019),
            '2019_asir': f"{asir_2019:.2f}",  # 添加ASIR
        ...
        for age_group in ['5-14 years', '15-49 years', '50-69 years', '70+ years']:
            ...
            if len(age_data) > 0:
                ...
                val_2019 = df_2019[(df_2019['sex_name'] == sex) & 
                                  (df_2019['age_name'] == age_group)]['val'].values[0]
                
                # 计算年龄组的ASIR
                age_asir_1990 = calculate_asir(age_data[age_data['year'] == 1990], 1990)
                age_asir_2019 = calculate_asir(age_data[age_data['year'] == 2019], 2019)
                
                aapc = calculate_aapc(val_1990, val_2019)
                ...
                results.append({
                    ...
                    '2019_cases': format_number(val_2019),
                    '2019_asir': f"{age_asir_2019:.2f}",  # 添加ASIR
                    'AAPC': f"{aapc:.2f}"
                })
...
def plot_table(china_df, g20_df):
    ...
    plt.title('Table 1. The incidence cases, age-standardised rates, and temporal trend of Thyroid cancer from 1990 to 2019',
             pad=20, fontsize=12)
    
    plt.savefig('/Users/caijianyu/go/src/code.byted.org/argos/gbd/output/thyroid_cancel.png',
                dpi=300, bbox_inches='tight')
    plt.close()

def main():
    # 读取发病数据
    df_incidence = pd.read_csv('/Users/caijianyu/go/src/code.byted.org/argos/gbd/data/raw/thyroid_cancer.csv')
    ...
    print("分析结果已保存为图表：/Users/caijianyu/go/src/code.byted.org/argos/gbd/output/thyroid_cancel.png")
